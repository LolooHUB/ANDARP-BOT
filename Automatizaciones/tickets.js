...
module.exports = {
    // Función para enviar el Panel Inicial (Ejecutada por el index.js en cada reinicio)
    ...
    async handleTicketInteractions(interaction) {
        ...
        if (interaction.isModalSubmit() && customId.startsWith('modal_t_')) {
            ...
            interaction.fields.fields.forEach(f => {
                embedInfo.addFields({ name: f.customId.replace('_', ' ').toUpperCase(), value: f.value });
            });
            ...
            if (logChan) await logChan.send(`📥 **${user.tag}** ABRIÓ EL TICKET #${ticketId}`);
        ...
        if (customId === 'ticket_reclamar') {
            ...
            const logChan = guild.channels.cache.get('1476799509207060551');
            if (logChan) await logChan.send(`📌 **${interaction.user.tag}** RECLAMÓ EL TICKET ${interaction.channel.name}`);
        }
